...
import { BiasBadge, DataSourceBadge, LowCoverageFlag } from '@/components/data/badges';
        ...
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-line">
              ...
              <Th className="hidden min-w-[110px] xl:table-cell">{t('provenance.column')}</Th>
            ...
            {visible.map((row, i) => (
              <Fragment key={row.symbol}>
                ...
                <tr className="group border-b border-line last:border-0">
                  ...
                  <Td className={cn('hidden xl:table-cell', compact ? 'py-2.5' : 'py-3.5')}>
                    <DataSourceBadge
                      isFullyReal={row.isFullyReal}
                      realShare={row.realShare}
                      realLabel={t('provenance.real')}
                      hybridLabel={t('provenance.hybrid')}
                      title={row.isFullyReal ? t('provenance.realHint') : t('provenance.hybridHint')}
                    />
                  </Td>
                  <Td
                    className={cn(
