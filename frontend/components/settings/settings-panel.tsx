...
export function SettingsPanel({
  ...
  /** Reported by the API from row provenance, not hardcoded in the UI. */
  dataSource: string;
}) {
  ...
  return (
    <div className="max-w-[720px] space-y-8">
      ...
      <Group title={t('theme.title')} hint={t('theme.hint')}>
        <ThemeSegmented />
      </Group>
      ...
      <Group title={t('settings.about')} hint={t('settings.aboutHint')}>
        <dl className="divide-y divide-line overflow-hidden rounded-[10px] border border-line bg-canvas">
          ...
          <Row label={t('settings.dataSource')} value={dataSource} />
        </dl>
      </Group>
    </div>
  );
